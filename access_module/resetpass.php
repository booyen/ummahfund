...
note: tukar password form HTML, hanya jika user exist
status: function settle, reroute balik ke main after user login
!-->
<?php
...
if(isset($_GET['id']) && isset($_GET['code']))
{
	...
	$stmt = $user->runQuery("SELECT * FROM um_users WHERE userID=:uid AND tokenCode=:token");
	...
	if($stmt->rowCount() == 1)
	{
		if(isset($_POST['btn-reset-pass']))
		{
			...
			{
				$password = md5($cpass);
				$stmt = $user->runQuery("UPDATE um_users SET userPass=:upass WHERE userID=:uid");
